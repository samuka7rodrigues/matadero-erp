...
import { presupuestoSchema, type PresupuestoFormData } from '@/types/finanzas';
import type { Cliente } from '@/types/database';
import { createPresupuesto } from '@/actions/finanzas';
...
export function PresupuestoForm({ clientes }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('presupuestos.new')}</CardTitle>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor="numero">{t('presupuestos.numero')} *</Label>
            <Input id="numero" {...register('numero')} placeholder="PRE-2026-001" className="font-mono" />
            {errors.numero && <p className="text-xs text-destructive">{errors.numero.message}</p>}
          </div>

          <div className="space-y-2">
            <Label htmlFor="titulo">{t('presupuestos.titulo')} *</Label>
            <Input id="titulo" {...register('titulo')} />
            {errors.titulo && <p className="text-xs text-destructive">{errors.titulo.message}</p>}
          </div>

          <div className="space-y-2">
            <Label htmlFor="cliente_id">{t('presupuestos.cliente')}</Label>
          ...
          <div className="space-y-2">
            <Label htmlFor="estado">{t('presupuestos.estado')}</Label>
            <Select id="estado" {...register('estado')}>
              <option value="borrador">Borrador</option>
              <option value="enviado">Enviado</option>
              <option value="aceito">Aceite</option>
              <option value="rechazado">Rejeitado</option>
            </Select>
          </div>

          <div className="space-y-2">
            <Label htmlFor="data">{t('presupuestos.data')} *</Label>
            <Input id="data" type="date" {...register('data')} />
            {errors.data && <p className="text-xs text-destructive">{errors.data.message}</p>}
          </div>

          <div className="space-y-2">
            <Label htmlFor="validade">{t('presupuestos.validade')}</Label>
            <Input id="validade" type="date" {...register('validade')} />
          </div>

          <div className="space-y-2 md:col-span-2">
            <Label htmlFor="observacoes">{t('presupuestos.observacoes')}</Label>
          ...
          {fields.map((field, index) => (
            <div key={field.id} className="grid grid-cols-12 items-end gap-2 rounded-md border p-3">
              <div className="col-span-12 space-y-2 md:col-span-5">
                <Label htmlFor={`itens.${index}.descricao`}>{t('presupuestos.descricao')} *</Label>
              ...
              <div className="col-span-4 space-y-2 md:col-span-2">
                <Label htmlFor={`itens.${index}.quantidade`}>{t('presupuestos.quantidade')}</Label>
                <Input id={`itens.${index}.quantidade`} type="number" step="0.01" {...register(`itens.${index}.quantidade`)} />
              </div>
              <div className="col-span-4 space-y-2 md:col-span-2">
                <Label htmlFor={`itens.${index}.preco_unitario`}>{t('presupuestos.precoUnitario')}</Label>
              ...
              <div className="col-span-3 space-y-2 md:col-span-2">
                <Label htmlFor={`itens.${index}.iva_pct`}>{t('presupuestos.ivaPct')}</Label>
          ...
          <div className="flex justify-end border-t pt-3 text-sm">
            <div className="w-full max-w-xs space-y-1">
              <div className="flex justify-between">
                <span>{t('presupuestos.total')}</span>
                <span className="font-bold">{(totais.base + totais.iva).toLocaleString()} €</span>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <div className="flex justify-end gap-3">
        <Button type="button" variant="outline" onClick={() => router.push('/presupuestos')}>
          {tc('cancel')}
        </Button>
